...
export const fetchChannelDetails = async (channelId) => {
  const API_URL = `https://youtube-v31.p.rapidapi.com/channels?part=snippet,statistics&id=${channelId}`;
  ...
  try {
    const response = await fetch(API_URL, options);
    if (!response.ok) throw new Error("Failed to fetch channel details", error);
